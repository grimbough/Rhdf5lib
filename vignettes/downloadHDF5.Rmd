...
download.file(url = "https://support.hdfgroup.org/ftp/HDF5/releases/hdf5-1.10/hdf5-1.10.5/src/hdf5-1.10.5.tar.bz2", dest = hdf5_source)
...
system2("tar", args = c("-C", tempdir(), "-czf", file.path(tempdir(), "hdf5small_cxx_hl_1.10.5.tar.gz"), "hdf5"))
```

Finally copy this to the `/tmp/` so it is easy to find, inspect, and move to the package directory if correct.

```{r}
if(file.exists("/tmp/hdf5small_cxx_hl_1.10.5.tar.gz")) { file.remove("/tmp/hdf5small_cxx_hl_1.10.5.tar.gz") }
file.copy(file.path(tempdir(), "hdf5small_cxx_hl_1.10.5.tar.gz"), to = "/tmp/")
```
