...
system2("tar", args = c("-C", tempdir(), "-czf", file.path(tempdir(), "hdf5small_cxx_1.10.4.tar.gz"), "hdf5"))
```

Finally copy this to the `/tmp/` so it is easy to find, inspect, and move to the package directory if correct.

```{r}
if(file.exists("/tmp/hdf5small_cxx_1.10.4.tar.gz")) { file.remove("/tmp/hdf5small_cxx_1.10.4.tar.gz") }
file.copy(file.path(tempdir(), "hdf5small_cxx_1.10.4.tar.gz"), to = "/tmp/")
```
